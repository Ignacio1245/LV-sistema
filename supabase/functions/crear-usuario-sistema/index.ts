...
Deno.serve(async (request) => {
  ...
  if (usuarioAuthExistente) {
    const { data: dataActualizada, error: errorActualizacion } =
      await supabaseAdmin.auth.admin.updateUserById(usuarioAuthExistente.id, {
        password,
        email_confirm: true
      });

    if (errorActualizacion) {
      return responderJson({ error: errorActualizacion.message }, 400);
    }
    ...
    return responderJson({
      ...
      id: dataActualizada.user?.id || usuarioAuthExistente.id
    });
  }
  ...
  if (error) {
    return responderJson({ error: error.message || "No se pudo crear el acceso." }, 400);
  }
