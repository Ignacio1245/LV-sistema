  ...
  const lineasProductos =
    datosPedido.items.map(function (item) {
      return "- " + item.cantidad + " x " +
        item.nombre + " (" + formatearDinero(item.cantidad * item.precio) + ")";
    });
...
const pedidoCatalogo = {
  ...
  items: [
    { nombre: "Azucar", cantidad: 2, precio: 1500 },
    { nombre: "Yerba", cantidad: 1, precio: 2200 }
  ],
  total: 5200
};
...
assert.ok(mensajeCatalogo.includes("- 2 x Azucar"));
...
const pedidoVendedor = {
  ...
  items: [
    { nombre: "Aceite", cantidad: 3, precio: 1800 }
  ],
  total: 5400
};
