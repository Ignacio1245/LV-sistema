...
function credencialesLocalesInicialesValidas(email, password) {
  return normalizarEmailLogin(email) === normalizarEmailLogin(EMAIL_ACCESO_LOCAL_INICIAL) &&
    password === CLAVE_ACCESO_LOCAL_INICIAL;
}

function obtenerMensajeAccesoLocalInicial() {
  return "Acceso inicial local: " + EMAIL_ACCESO_LOCAL_INICIAL +
...
async function iniciarSesionSistema(email, password) {
  let etapaLogin =
    "validacion";
  ...
  try {
    ...
    if (iniciarSesionLocalInicial(email, password)) {
      return;
    }
    ...
    if (!usuario) {
      await cerrarSesionSupabase();
      throw new Error("El login de Supabase existe, pero ese email no esta creado en Usuarios del sistema.");
    }
    ...
    throw new Error("No se pudo ingresar en etapa: " + etapaLogin + ". " + (error.message || "Error desconocido"));
  }
}
