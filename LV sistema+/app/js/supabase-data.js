...
async function cargarAuditoriaDesdeSupabase() {
  try {
    ...
    if (auditoriaSupabase.length === 0) {
      return crearResultadoCargaSupabase("auditoria", 0);
    }
...
async function cargarDatosBaseDesdeSupabase() {
  try {
    ...
    if (zonasSupabase.length > 0) {
      zonas = zonasSupabase;
    }

    if (rubrosSupabase.length > 0) {
      rubros = rubrosSupabase;
    }

    if (proveedoresSupabase.length > 0) {
    ...
    if (listasSupabase.length > 0) {
      listasPrecios = listasSupabase;
    }

    if (proveedorPagosSupabase.length > 0) {
      proveedorPagos = proveedorPagosSupabase;
    }

    if (comprasSupabase.length > 0) {
