...
function exportarRespaldoSistema() {
  if (!tienePermiso("configuracion")) {
    alert("Tu rol no tiene permiso para exportar respaldos.");
    return;
  }
...
function obtenerResumenValidacionRespaldo(respaldo) {
  ...
  const listasFaltantes =
    listasObligatorias.filter(function (nombreLista) {
      return !Array.isArray(respaldo.datos[nombreLista]);
    });

  if (listasFaltantes.length > 0) {
    return {
      valido: false,
      mensaje: "El respaldo esta incompleto. Faltan listas: " + listasFaltantes.join(", ")
    };
  }

  if (!respaldo.datos.configuracion || typeof respaldo.datos.configuracion !== "object") {
    return {
      valido: false,
      mensaje: "El respaldo esta incompleto. Falta configuracion."
    };
  }

  if (!respaldo.datos.roles || typeof respaldo.datos.roles !== "object") {
    return {
      valido: false,
      mensaje: "El respaldo esta incompleto. Faltan roles."
    };
  }
...
async function restaurarRespaldoSistema() {
  if (!tienePermiso("configuracion")) {
    alert("Tu rol no tiene permiso para restaurar respaldos.");
    return;
  }
