...
function seleccionarFiltroEstadoProveedores(filtro) {
  const filtroSeguro =
    ["activos", "inactivos", "todos"].includes(filtro) ? filtro : "activos";

  filtroEstadoProveedores = filtroSeguro;

  dom.providerStatusFilterButtons.forEach(function (boton) {
    boton.classList.toggle(
      "active",
      boton.dataset.providerStatusFilter === filtroSeguro
    );
  });
}
...
function pagoProveedorCoincideMedio(pago, medioSeleccionado) {
  const medioPago =
    pago && pago.medio ? pago.medio : "";
  const mediosConocidos = [
    "EFECTIVO",
    "TRANSFERENCIA",
    "CHEQUE",
    "CUENTA_CORRIENTE"
  ...
  if (!medioSeleccionado || medioSeleccionado === "TODOS") {
    return true;
  }

  if (medioSeleccionado === "OTRO") {
    return medioPago !== "" && !mediosConocidos.includes(medioPago);
  }

  return medioPago === medioSeleccionado;
}
...
function obtenerPagosProveedoresFiltrados() {
  ...
  return proveedorPagos.filter(function (pago) {
    ...
    const coincideMedio =
      pagoProveedorCoincideMedio(pago, medio);
