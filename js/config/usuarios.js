...
function normalizarUsuarioAccesoSistema(usuario) {
    return normalizarTexto(String(usuario || ""))
        .replace(/[^a-z0-9]+/g, ".")
        .replace(/^\.+|\.+$/g, "");
}

function obtenerEmailInternoUsuarioSistema(usuarioOEmail) {
    const valor =
    ...
    const usuarioNormalizado =
        normalizarUsuarioAccesoSistema(valor);
...
function renderizarUsuarioActual() {
    ...
    renderizarFiltroRolesUsuariosSistema();
...
function obtenerTextoAccesoUsuarioSistema(usuario) {
    const email =
...
function renderizarFiltroRolesUsuariosSistema() {
    ...
    dom.usuarioSistemaRolFiltro.value =
        rolActual === "TODOS" || ROLES[rolActual] ? rolActual : "TODOS";
}

function obtenerUsuariosSistemaFiltrados() {
    const textoBusqueda =
        normalizarTexto(dom.buscarUsuarioSistemaInput ? dom.buscarUsuarioSistemaInput.value : "");
    const rolFiltro =
        dom.usuarioSistemaRolFiltro ? dom.usuarioSistemaRolFiltro.value : filtroRolUsuariosSistema;

    return usuariosSistema.filter(function (usuario) {
        const coincideRol =
            !rolFiltro || rolFiltro === "TODOS" || usuario.rol === rolFiltro;
        const textoUsuario =
            [
                usuario.codigo,
                usuario.nombre,
                usuario.email,
                obtenerTextoAccesoUsuarioSistema(usuario),
                usuario.rol
            ].join(" ");
        const coincideTexto =
            textoBusqueda === "" || normalizarTexto(textoUsuario).includes(textoBusqueda);
...
function renderizarUsuariosSistema() {
    ...
    dom.usuariosSistemaTable.innerHTML =
        usuariosFiltrados.map(function (usuario) {
            const estadoTexto =
      ...
      <tr>
        ...
        <td>
          ...
          <small>${usuario.email || "Sin acceso asignado"}</small>
        </td>
        <td>${obtenerTextoAccesoUsuarioSistema(usuario)}</td>
        <td>${usuario.rol}</td>
        <td>
...
async function agregarUsuarioSistema(event) {
    ...
    if (email === "") {
        alert("Ingrese un usuario o email valido.");
        return;
    }
...
async function guardarEdicionUsuarioSistema(codigo, nombre, email, rol, password) {
    ...
    if (password) {
        try {
            ...
            console.error("No se pudo crear acceso de usuario:", error);
            alert("No se pudo actualizar el acceso de login: " + (error.message || "error"));
            return;
        }
    }
