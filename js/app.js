const dom = {
  ...
  linksPage: document.querySelector("#linksPage"),
  ...
  buscarUsuarioSistemaInput: document.querySelector("#buscarUsuarioSistemaInput"),
  usuarioSistemaRolFiltro: document.querySelector("#usuarioSistemaRolFiltro"),
  verTodosUsuariosSistemaButton: document.querySelector("#verTodosUsuariosSistemaButton"),
  ...
  syncPendingDataButton: document.querySelector("#syncPendingDataButton"),
  refreshOnlineDataButton: document.querySelector("#refreshOnlineDataButton"),
  ...
  listaPreciosGeneradorRubros: document.querySelector("#listaPreciosGeneradorRubros"),
  listaPreciosGeneradorMarcas: document.querySelector("#listaPreciosGeneradorMarcas"),
  listaPreciosGeneradorListas: document.querySelector("#listaPreciosGeneradorListas"),
  generarListaPreciosButton: document.querySelector("#generarListaPreciosButton"),
  imprimirListaPreciosButton: document.querySelector("#imprimirListaPreciosButton"),
  exportarListaPreciosButton: document.querySelector("#exportarListaPreciosButton"),
  limpiarListaPreciosButton: document.querySelector("#limpiarListaPreciosButton"),
  listaPreciosGeneradaHead: document.querySelector("#listaPreciosGeneradaHead"),
  listaPreciosGeneradaBody: document.querySelector("#listaPreciosGeneradaBody"),
  ...
  providerStatusFilterButtons: document.querySelectorAll("[data-provider-status-filter]"),
  ...
  informeDiasResumen: document.querySelector("#informeDiasResumen"),
  informeVentasPorDiaTable: document.querySelector("#informeVentasPorDiaTable"),
  informeDetallePedidosDiaTable: document.querySelector("#informeDetallePedidosDiaTable"),
  ...
  vendedorPasswordInput: document.querySelector("#vendedorPasswordInput"),
  ...
  clientesImportacionPreview: document.querySelector("#clientesImportacionPreview"),
  ...
  dashboardEstadoArranquePanel: document.querySelector("#dashboardEstadoArranquePanel"),
  dashboardEstadoArranqueTitulo: document.querySelector("#dashboardEstadoArranqueTitulo"),
  dashboardEstadoArranqueDetalle: document.querySelector("#dashboardEstadoArranqueDetalle"),
  dashboardEstadoArranqueLista: document.querySelector("#dashboardEstadoArranqueLista"),
  ...
  cobranzasPendientesVendedorBloque: document.querySelector("#cobranzasPendientesVendedorBloque"),
  cobranzasPendientesVendedorLista: document.querySelector("#cobranzasPendientesVendedorLista"),
  cobranzasPendientesVendedorEstado: document.querySelector("#cobranzasPendientesVendedorEstado"),
  cobranzasPendientesVendedorCantidad: document.querySelector("#cobranzasPendientesVendedorCantidad"),
  actualizarCobranzasVendedorButton: document.querySelector("#actualizarCobranzasVendedorButton"),
  cuentaClienteEstadoBloque: document.querySelector("#cuentaClienteEstadoBloque"),
  cuentaClienteBusquedaInput: document.querySelector("#cuentaClienteBusquedaInput"),
  cuentaClienteSelect: document.querySelector("#cuentaClienteSelect"),
  cuentaClienteDesdeInput: document.querySelector("#cuentaClienteDesdeInput"),
  cuentaClienteHastaInput: document.querySelector("#cuentaClienteHastaInput"),
  actualizarCuentaClienteButton: document.querySelector("#actualizarCuentaClienteButton"),
  imprimirCuentaClienteButton: document.querySelector("#imprimirCuentaClienteButton"),
  cuentaClienteEstadoResumen: document.querySelector("#cuentaClienteEstadoResumen"),
  cuentaClienteEstadoTable: document.querySelector("#cuentaClienteEstadoTable"),
  ...
  productosImportacionPreview: document.querySelector("#productosImportacionPreview"),
  ...
  pedidoRapidoModal: document.querySelector("#pedidoRapidoModal"),
  pedidoRapidoTitulo: document.querySelector("#pedidoRapidoTitulo"),
  pedidoRapidoProductoDetalle: document.querySelector("#pedidoRapidoProductoDetalle"),
  pedidoRapidoCantidadForm: document.querySelector("#pedidoRapidoCantidadForm"),
  pedidoRapidoBonificacionForm: document.querySelector("#pedidoRapidoBonificacionForm"),
  pedidoRapidoCantidadInput: document.querySelector("#pedidoRapidoCantidadInput"),
  pedidoRapidoBonificacionInput: document.querySelector("#pedidoRapidoBonificacionInput"),
  pedidoRapidoPrecioPreview: document.querySelector("#pedidoRapidoPrecioPreview"),
  pedidoRapidoSubtotalPreview: document.querySelector("#pedidoRapidoSubtotalPreview"),
  cerrarPedidoRapidoModal: document.querySelector("#cerrarPedidoRapidoModal"),
  cancelarPedidoRapidoButton: document.querySelector("#cancelarPedidoRapidoButton"),
  pedidoRapidoVolverCantidadButton: document.querySelector("#pedidoRapidoVolverCantidadButton"),
  pedidoRapidoSinBonificacionButton: document.querySelector("#pedidoRapidoSinBonificacionButton"),
...
function obtenerClaveClienteDuplicadoDashboard(cliente, tipo) {
  if (tipo === "telefono") {
    const telefono =
      String(cliente.telefono || cliente.telefonoMovil || cliente.telefonoParticular || "")
        .replace(/\D/g, "");

    return telefono.length >= 6 ? "telefono:" + telefono : "";
  }

  const nombre =
    normalizarTexto(cliente.nombre || cliente.nombreFantasia || cliente.razonSocial || "");
  const direccion =
    normalizarTexto(cliente.direccion || "");

  if (!nombre || !direccion || nombre === "consumidor final") {
    return "";
  }

  return "nombre-direccion:" + nombre + "|" + direccion;
}

function obtenerPosiblesDuplicadosClientesDashboard() {
  const grupos = {};

  clientes.filter(clienteActivo).forEach(function (cliente) {
    ["telefono", "nombre-direccion"].forEach(function (tipo) {
      const clave =
        obtenerClaveClienteDuplicadoDashboard(cliente, tipo);
      ...
      grupos[clave].clientes.push(cliente);
    });
  });
  ...
  return Object.keys(grupos).map(function (clave) {
    ...
    return grupo.clientes.length > 1;
  }).slice(0, 3).map(function (grupo) {
    const clientesGrupo =
      grupo.clientes.slice(0, 3).map(function (cliente) {
        return (cliente.codigo || "-") + " - " + (cliente.nombre || "Sin nombre");
      });

    return {
      tipo: "Cliente",
      texto: "Posible duplicado",
      detalle: clientesGrupo.join(" / ") +
        (grupo.tipo === "telefono" ? " | mismo telefono" : " | mismo nombre y direccion")
    };
  });
}
function obtenerDatosIncompletosParaDashboard() {
  const datosIncompletos = [];
  const listasActivasNormalizadas =
    typeof obtenerNombresListasPreciosActivas === "function"
      ? obtenerNombresListasPreciosActivas().map(function (nombreLista) {
        return normalizarNombreListaPrecio(nombreLista);
      })
      : [];
  ...
  clientes.filter(clienteActivo).forEach(function (cliente) {
    ...
    if (textoVacio(cliente.listaPrecios)) {
      camposFaltantes.push("lista precios");
    } else if (listasActivasNormalizadas.length > 0 &&
        !listasActivasNormalizadas.includes(normalizarNombreListaPrecio(cliente.listaPrecios))) {
      camposFaltantes.push("lista inexistente");
    }
  ...
  datosIncompletos.push(...obtenerPosiblesDuplicadosClientesDashboard());
  ...
  productos.filter(productoActivo).forEach(function (producto) {
    ...
    if (typeof obtenerProblemasPrecioProducto === "function") {
      camposFaltantes.push(...obtenerProblemasPrecioProducto(producto).slice(0, 4));
    } else if ((Number(producto.precio) || 0) <= 0) {
...
function restablecerDatosLocalesSistema() {
  ...
  if (typeof exportarRespaldoSistema === "function") {
    exportarRespaldoSistema();
  }
  ...
  if (typeof limpiarPedidoActualLocal === "function") {
    limpiarPedidoActualLocal();
  }
...
function crearRevisionArranqueSistema(nivel, texto, detalle) {
  return {
    nivel: nivel,
    texto: texto,
    detalle: detalle
...
function obtenerCantidadProductosSinPrecioValido() {
  return productos.filter(productoActivo).filter(function (producto) {
    if (typeof obtenerProblemasPrecioProducto === "function") {
      return obtenerProblemasPrecioProducto(producto).length > 0;
    }
...
function obtenerCantidadUsuariosVendedoresActivos() {
  ...
  return usuariosSistema.filter(function (usuario) {
    return usuario &&
      ...
      normalizarTexto(usuario.rol || "") === "vendedor";
  }).length;
}
...
function obtenerRevisionesArranqueSistema(datosIncompletos, productosCriticos) {
  ...
  const clientesActivos =
    clientes.filter(clienteActivo).length;
  const productosActivos =
    productos.filter(productoActivo).length;
  const rubrosActivos =
    rubros.filter(rubroActivo).length;
  const zonasActivas =
    zonas.filter(zonaActiva).length;
  const vendedoresActivos =
    obtenerVendedoresComerciales().filter(function (vendedor) {
      return vendedor.activo !== false;
    }).length;
  const productosSinPrecio =
    obtenerCantidadProductosSinPrecioValido();
  ...
  const sesionOnlineActiva =
    typeof usuarioSupabaseAutenticado === "function" && usuarioSupabaseAutenticado();

  if (!supabaseConfigurado) {
    revisiones.push(crearRevisionArranqueSistema(
      "critico",
      "Supabase sin configurar",
      "sin guardado compartido"
    ));
  } else if (!sesionOnlineActiva) {
    revisiones.push(crearRevisionArranqueSistema(
      "critico",
      "Sesion online cerrada",
      "ingresar antes de operar"
    ));
  }

  if (hayPendientesSupabase) {
    revisiones.push(crearRevisionArranqueSistema(
      "critico",
      "Cambios pendientes",
      "subir a Supabase"
  ...
  if (productosSinPrecio > 0) {
    revisiones.push(crearRevisionArranqueSistema(
      "critico",
      "Precios",
      productosSinPrecio + " producto" + (productosSinPrecio === 1 ? "" : "s") + " a revisar"
    ));
  }
  ...
  if (vendedoresActivos > 0 && obtenerCantidadUsuariosVendedoresActivos() === 0) {
    revisiones.push(crearRevisionArranqueSistema(
      "aviso",
      "Accesos vendedores",
  ...
  if (posiblesDuplicados > 0) {
    revisiones.push(crearRevisionArranqueSistema(
      "aviso",
      "Clientes duplicados",
      posiblesDuplicados + " grupo" + (posiblesDuplicados === 1 ? "" : "s") + " posible" + (posiblesDuplicados === 1 ? "" : "s")
    ));
  }
...
function obtenerResumenArranqueSistema(revisiones) {
  const criticos =
    revisiones.filter(function (revision) {
      return revision.nivel === "critico";
    }).length;
  const avisos =
    revisiones.filter(function (revision) {
      return revision.nivel === "aviso";
    }).length;

  if (criticos > 0) {
    return {
      estado: "critico",
      titulo: "Revisar antes de operar",
      detalle: criticos + " punto" + (criticos === 1 ? "" : "s") + " critico" + (criticos === 1 ? "" : "s") + " pendiente" + (criticos === 1 ? "" : "s")
    };
  }
  ...
  if (avisos > 0) {
    return {
      estado: "aviso",
      titulo: "Listo con avisos",
      detalle: avisos + " aviso" + (avisos === 1 ? "" : "s") + " para revisar"
    };
  }
  ...
  return {
    ...
    detalle: "Sin alertas de arranque"
  };
}

function renderizarEstadoArranqueSistema(datosIncompletos, productosCriticos) {
  if (!dom.dashboardEstadoArranquePanel || !dom.dashboardEstadoArranqueLista) {
    return;
  }

  const revisiones =
    obtenerRevisionesArranqueSistema(datosIncompletos, productosCriticos);
  const resumen =
    obtenerResumenArranqueSistema(revisiones);

  dom.dashboardEstadoArranquePanel.classList.remove("estado-listo", "estado-aviso", "estado-critico");
  dom.dashboardEstadoArranquePanel.classList.add("estado-" + resumen.estado);
  dom.dashboardEstadoArranqueTitulo.textContent =
    resumen.titulo;
  dom.dashboardEstadoArranqueDetalle.textContent =
    resumen.detalle;

  if (revisiones.length === 0) {
    dom.dashboardEstadoArranqueLista.innerHTML =
      crearFilaDashboard("Arranque", "listo para cargar datos reales");
    return;
  }
...
function renderizarCentroControlDashboard() {
  ...
  renderizarEstadoArranqueSistema(datosIncompletos, productosCriticos);
...
function mostrarSeccionCuenta(seccion) {
  ...
  if (dom.cobranzasPendientesVendedorBloque) {
    dom.cobranzasPendientesVendedorBloque.classList.toggle("hidden", seccionActual !== "cobranzas");
  }
  if (dom.cuentaClienteEstadoBloque) {
    dom.cuentaClienteEstadoBloque.classList.toggle("hidden", seccionActual !== "estado");
  }
  ...
  if (seccionActual === "cobranzas") {
    cargarCobranzasPendientesVendedor();
  }

  if (seccionActual === "estado") {
    prepararCuentaClienteEstado();
    actualizarCuentaClienteEstado();
    dom.cuentaClienteBusquedaInput.focus();
  }
...
function normalizarEmailAccesoVendedor(email) {
  if (typeof normalizarEmailUsuarioSistema === "function") {
    return normalizarEmailUsuarioSistema(email);
  }

  return String(email || "").trim().toLowerCase();
}

function obtenerEmailAccesoVendedorDesdeFormulario(nombreVendedor) {
  const usuarioIngresado =
    dom.vendedorEmailInput.value.trim() || nombreVendedor;

  if (typeof obtenerEmailInternoUsuarioSistema === "function") {
    return obtenerEmailInternoUsuarioSistema(usuarioIngresado);
  }

  return usuarioIngresado.includes("@")
    ? usuarioIngresado.toLowerCase()
    : usuarioIngresado.toLowerCase().replace(/\s+/g, ".") + "@lv.local";
}

function obtenerUsuarioSistemaVendedorPorEmail(emailAcceso) {
  const emailNormalizado =
    normalizarEmailAccesoVendedor(emailAcceso);

  return usuariosSistema.find(function (usuario) {
    return normalizarEmailAccesoVendedor(usuario.email) === emailNormalizado;
  });
}

function obtenerTextoUsuarioMovilVendedor(emailAcceso) {
  const email =
    String(emailAcceso || "").trim();

  if (email.endsWith("@lv.local")) {
    return email.replace("@lv.local", "");
  }
...
async function guardarUsuarioMovilDeVendedor(nombreVendedor, emailAcceso, password, vendedorActivo) {
  if (!tienePermiso("configuracion")) {
    alert("Tu rol no tiene permiso para administrar vendedores.");
    return false;
  }
  ...
  const usuarioExistente =
    obtenerUsuarioSistemaVendedorPorEmail(emailAcceso);
  ...
  if (
    ...
    alert("Para crear accesos de vendedor tenes que estar conectado a Supabase.");
    return false;
  }

  if ((esUsuarioNuevo || passwordIngresado) && typeof crearAccesoUsuarioSupabase === "function") {
    try {
      await crearAccesoUsuarioSupabase(emailAcceso, passwordIngresado);
    } catch (error) {
      console.error("No se pudo crear acceso movil del vendedor:", error);
      alert("No se pudo crear el acceso movil: " + (error.message || "error"));
      return false;
    }
  }
  ...
  if (!usuarioExistente) {
    usuariosSistema.push(usuarioMovil);
  }
    ...
    if (!usuarioExistente) {
      usuariosSistema =
        usuariosSistema.filter(function (usuario) {
          return usuario.codigo !== usuarioMovil.codigo;
        });
      guardarUsuariosSistema();
    }

    alert("No se pudo guardar el usuario movil en Supabase.");
    return false;
  }
...
function limpiarFormularioVendedor() {
  ...
  if (dom.vendedorPasswordInput) {
    dom.vendedorPasswordInput.value = "";
  }
  dom.vendedorSubmitButton.textContent = "Agregar vendedor";
  dom.cancelarEdicionVendedorButton.classList.add("hidden");
}

async function guardarVendedorDesdeFormulario(event) {
  ...
  const emailAcceso =
    obtenerEmailAccesoVendedorDesdeFormulario(nombre);
  const passwordMovil =
    dom.vendedorPasswordInput ? dom.vendedorPasswordInput.value : "";
  ...
  if (vendedorEditando !== null) {
    ...
    const nombreAnterior =
      vendedor.nombre;
    const usuarioMovilGuardado =
      await guardarUsuarioMovilDeVendedor(
        nombre,
        emailAcceso,
        passwordMovil,
    ...
    if (!vendedorGuardadoOnline && puedeGuardarOperacionEnSupabase()) {
  ...
  const vendedorGuardadoOnline =
    await guardarVendedorOperacionSupabase(nuevoVendedor);

  if (!vendedorGuardadoOnline && puedeGuardarOperacionEnSupabase()) {
    alert("No se pudo guardar el vendedor en Supabase. Revisa la conexion y permisos.");
    return;
  }
...
function editarVendedor(codigo) {
  if (!tienePermiso("configuracion")) {
    alert("Tu rol no tiene permiso para editar vendedores.");
    return;
  }
  ...
  dom.vendedorEmailInput.value =
    obtenerTextoUsuarioMovilVendedor(vendedor.email);
      ...
      vendedores.map(function (usuario) {
        ...
        return `
          <tr>
            ...
            <td>${obtenerTextoUsuarioMovilVendedor(usuario.email)}</td>
            <td>${usuario.tipo || "-"}</td>
...
function actualizarVistaBotonSincronizacionPendiente() {
  if (!dom.syncPendingDataButton) {
    return;
  }

  const cantidadPendiente =
    typeof obtenerCantidadTiposSincronizacionPendientes === "function"
      ? obtenerCantidadTiposSincronizacionPendientes()
      : 0;

  dom.syncPendingDataButton.classList.toggle("hidden", cantidadPendiente === 0);
  dom.syncPendingDataButton.textContent =
    cantidadPendiente > 0
      ? "Subir pendientes (" + cantidadPendiente + ")"
      : "Subir pendientes";
}
...
async function cargarTodoDesdeSupabase() {
  const cargarDatosOnline = async function () {
    return {
  ...
  const resultado =
    typeof pausarSincronizacionAutomatica === "function"
      ? await pausarSincronizacionAutomatica(cargarDatosOnline)
...
function renderizarPantallasDespuesDeActualizarDatos() {
...
async function actualizarDatosDesdeSupabaseManual() {
  if (typeof haySincronizacionPendiente === "function" && haySincronizacionPendiente()) {
    if (typeof sincronizarCambiosPendientesSupabase === "function") {
      await sincronizarCambiosPendientesSupabase();
    }
  }

  await ejecutarAccionSupabase(
    "Actualizar datos Supabase",
    async function () {
      const resultado =
...
async function sincronizarCambiosPendientesManual() {
  await ejecutarAccionSupabase(
    "Subir pendientes Supabase",
    async function () {
      if (typeof sincronizarCambiosPendientesSupabase !== "function") {
        throw new Error("No esta disponible la sincronizacion de pendientes.");
      }
...
function puedeActualizarOnlineAlCambiarApartado() {
  if (actualizandoDatosPorCambioDeApartado) {
    return false;
  }

  if (Date.now() - ultimaActualizacionOnlinePorCambioDeApartado < intervaloMinimoActualizacionApartado) {
    return false;
  }
  ...
  // Evita pisar un pedido abierto con datos recien escritos y todavia sin guardar.
...
async function actualizarDatosOnlineAlCambiarApartado(nombrePagina) {
  if (!puedeActualizarOnlineAlCambiarApartado()) {
    return;
  }

  actualizandoDatosPorCambioDeApartado = true;
  ultimaActualizacionOnlinePorCambioDeApartado = Date.now();

  try {
    actualizarEstadoSincronizacionSupabase(
      "Actualizando datos online por cambio de apartado...",
      "sync-working"
    );
    ...
    const erroresResultado =
      Array.isArray(resultado.errores) ? resultado.errores : [];

    renderizarPantallasDespuesDeActualizarDatos();

    if (erroresResultado.length > 0) {
      actualizarEstadoSincronizacionSupabase(
        "Datos actualizados con avisos en " + nombrePagina + ": " + erroresResultado.join(" | "),
        "sync-error"
      );
      return;
    }
    ...
    actualizarEstadoSincronizacionSupabase(
      "Datos online actualizados al abrir " + nombrePagina + ".",
      "sync-ok"
    );
  } catch (error) {
    console.error("Actualizacion por cambio de apartado fallo:", error);
    actualizarEstadoSincronizacionSupabase(
      "No se pudieron actualizar datos online al cambiar de apartado.",
      "sync-error"
    );
  } finally {
    actualizandoDatosPorCambioDeApartado = false;
  }
}
...
function aplicarPermisosDeUsuario() {
  ...
  if (dom.refreshOnlineDataButton) {
    dom.refreshOnlineDataButton.style.display =
      sistemaAutenticado() ? "inline-flex" : "none";
  }
}

function obtenerBaseUrlSistema() {
...
function construirLinkSistema(ruta, incluirWhatsapp) {
  ...
  if (incluirWhatsapp && whatsapp) {
    url.searchParams.set("wsp", whatsapp);
  }

  return url.toString();
}

function renderizarLinksSistema() {
  document.querySelectorAll("[data-system-link]").forEach(function (link) {
    const url =
      construirLinkSistema(
        link.dataset.systemPath || "/",
        link.dataset.systemWhatsapp === "true"
      );
    const enlaceAbrir =
      document.querySelector('[data-system-open="' + link.id + '"]');
...
async function copiarLinkSistema(idLink) {
  const link =
...
function mostrarPagina(nombre) {
  ...
  const tituloPagina =
    paginaSolicitada;
  ultimaPaginaVisibleSistema =
    tituloPagina || nombre || "dashboard";
  ...
  dom.linksPage.classList.add("hidden");
  ...
  if (nombre === "links") {
    dom.linksPage.classList.remove("hidden");
    renderizarLinksSistema();
  }
...
function configurarEventos() {
  ...
  dom.quickActionButtons.forEach(function (button) {
    ...
    button.addEventListener("click", function () {
      ...
      if (button.dataset.quickAction === "revision-precios") {
        mostrarPagina("productos");

        if (dom.productosVistaInput) {
          dom.productosVistaInput.value = "revision_precios";
  ...
  document.querySelectorAll("[data-system-copy]").forEach(function (boton) {
    boton.addEventListener("click", function () {
      copiarLinkSistema(boton.dataset.systemCopy);
    });
  });

  document.addEventListener(
    "visibilitychange",
    actualizarDatosOnlineAlVolverAlSistema
  );
  ...
  if (dom.refreshOnlineDataButton) {
    dom.refreshOnlineDataButton.addEventListener(
      "click",
      actualizarDatosDesdeSupabaseManual
    );
  }

  if (dom.syncPendingDataButton) {
    dom.syncPendingDataButton.addEventListener(
      "click",
      sincronizarCambiosPendientesManual
    );
  }
  ...
  if (dom.buscarUsuarioSistemaInput) {
    dom.buscarUsuarioSistemaInput.addEventListener(
      "input",
      renderizarUsuariosSistema
    );
  }

  if (dom.usuarioSistemaRolFiltro) {
    dom.usuarioSistemaRolFiltro.addEventListener(
      "change",
      function () {
        filtroRolUsuariosSistema = dom.usuarioSistemaRolFiltro.value || "TODOS";
        renderizarUsuariosSistema();
      }
    );
  }

  if (dom.verTodosUsuariosSistemaButton) {
    dom.verTodosUsuariosSistemaButton.addEventListener(
      "click",
      function () {
        if (dom.buscarUsuarioSistemaInput) {
          dom.buscarUsuarioSistemaInput.value = "";
        }

        if (dom.usuarioSistemaRolFiltro) {
          dom.usuarioSistemaRolFiltro.value = "TODOS";
        }

        filtroRolUsuariosSistema = "TODOS";
        renderizarUsuariosSistema();
      }
    );
  }
  ...
  if (dom.generarListaPreciosButton) {
    dom.generarListaPreciosButton.addEventListener(
      "click",
      generarListaPrecios
    );
  }

  if (dom.imprimirListaPreciosButton) {
    dom.imprimirListaPreciosButton.addEventListener(
      "click",
      imprimirListaPreciosGenerada
    );
  }

  if (dom.exportarListaPreciosButton) {
    dom.exportarListaPreciosButton.addEventListener(
      "click",
      exportarListaPreciosCsv
    );
  }

  if (dom.limpiarListaPreciosButton) {
    dom.limpiarListaPreciosButton.addEventListener(
      "click",
      limpiarSeleccionGeneradorListasPrecios
    );
  }
  ...
  dom.providerStatusFilterButtons.forEach(function (boton) {
    boton.addEventListener("click", function () {
      seleccionarFiltroEstadoProveedores(boton.dataset.providerStatusFilter);
      renderizarProveedores();
    });
  });
  ...
  [
    dom.proveedorPagoFechaDesdeFiltro,
    dom.proveedorPagoFechaHastaFiltro,
    dom.proveedorPagoMedioFiltro
  ].forEach(function (input) {
    if (!input) {
      return;
    }

    input.addEventListener("input", renderizarPagosProveedores);
    input.addEventListener("change", renderizarPagosProveedores);
  });
  ...
  if (dom.actualizarCobranzasVendedorButton) {
    dom.actualizarCobranzasVendedorButton.addEventListener(
      "click",
      cargarCobranzasPendientesVendedor
    );
  }

  if (dom.cuentaClienteBusquedaInput) {
    dom.cuentaClienteBusquedaInput.addEventListener(
      "input",
      function () {
        prepararCuentaClienteEstado();
        actualizarCuentaClienteEstado();
  ...
  if (dom.cuentaClienteSelect) {
    dom.cuentaClienteSelect.addEventListener(
      "change",
      actualizarCuentaClienteEstado
    );
  }
  ...
  [
    dom.cuentaClienteDesdeInput,
    dom.cuentaClienteHastaInput
  ].forEach(function (input) {
    if (!input) {
      return;
    }
  ...
  if (dom.actualizarCuentaClienteButton) {
    dom.actualizarCuentaClienteButton.addEventListener(
      "click",
      actualizarCuentaClienteEstado
    );
  }

  if (dom.imprimirCuentaClienteButton) {
    dom.imprimirCuentaClienteButton.addEventListener(
      "click",
      imprimirCuentaClienteEstado
    );
  }
  ...
  dom.productoSearchInput.addEventListener(
    ...
    function (event) {
      ...
      if (productoSeleccionado) {
        ...
        iniciarCargaRapidaProducto(productoSeleccionado);
        return;
      }
  ...
  [dom.clientesImportacionArchivo, dom.clientesImportacionTexto].forEach(function (input) {
    if (!input) {
      return;
    }

    input.addEventListener("change", limpiarPrevisualizacionImportacionClientes);
    input.addEventListener("input", limpiarPrevisualizacionImportacionClientes);
  });
  ...
  [dom.productosImportacionArchivo, dom.productosImportacionTexto].forEach(function (input) {
    if (!input) {
      return;
    }

    input.addEventListener("change", limpiarPrevisualizacionImportacionProductos);
    input.addEventListener("input", limpiarPrevisualizacionImportacionProductos);
  });
  ...
  dom.pedidoRapidoCantidadForm.addEventListener(
    "submit",
    confirmarCantidadCargaRapidaPedido
  );

  dom.pedidoRapidoBonificacionForm.addEventListener(
    "submit",
    agregarProductoCargaRapidaPedido
  );

  dom.pedidoRapidoCantidadInput.addEventListener(
    "input",
    actualizarResumenCargaRapidaPedido
  );

  dom.pedidoRapidoBonificacionInput.addEventListener(
    "input",
    actualizarResumenCargaRapidaPedido
  );

  dom.pedidoRapidoVolverCantidadButton.addEventListener(
    "click",
    function () {
  ...
  dom.pedidoRapidoSinBonificacionButton.addEventListener(
    "click",
    agregarProductoSinBonificacionCargaRapida
  );

  dom.cancelarPedidoRapidoButton.addEventListener(
    "click",
    cerrarCargaRapidaPedido
  );

  dom.cerrarPedidoRapidoModal.addEventListener(
    "click",
    cerrarCargaRapidaPedido
  );

  dom.pedidoRapidoModal.addEventListener(
    "click",
    function (event) {
      if (event.target === dom.pedidoRapidoModal) {
        cerrarCargaRapidaPedido();
      }
    }
  );
  ...
  window.addEventListener(
    "beforeunload",
    advertirSalidaConPedidoSinGuardar
  );
  ...
  const botonNuevoPedidoTopbar =
    document.querySelector("#newSaleButton");

  if (botonNuevoPedidoTopbar) {
    botonNuevoPedidoTopbar.addEventListener("click", function () {
      abrirNuevoPedidoDesdeMenu();
    });
  }
}
...
async function iniciarApp() {
  ...
  if (typeof restaurarPedidoActualLocalSiCorresponde === "function") {
    restaurarPedidoActualLocalSiCorresponde();
  }
}
